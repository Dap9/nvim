...
---@return boolean ok, plugins.treesitter.utils.treesitter.Health health
function M.check()
  ...
  ---@class plugins.treesitter.utils.treesitter.Health: table<string,boolean>
  local ret = {
...
function M.build(cb)
  M.ensure_treesitter_cli(function(_, err)
    ...
    if ok then
      ...
      vim.health.report_error(lines, { title = "Treesitter" })
    end
  end)
end
...
function M.ensure_treesitter_cli(cb)
  ...
  mr.refresh(function()
    ...
    if not p:is_installed() then
      vim.health.info("Installing `tree-sitter-cli` with `mason.nvim`...")
      p:install(
        nil,
        vim.schedule_wrap(function(success)
          if success then
            vim.health.info("Installed `tree-sitter-cli` with `mason.nvim`.")
            cb(true)
          else
